...
//直接插入排序
void stinsort(node r[MAXSIZE], int n)	//传入关键字列表
{
	clock_t start, stop;	//定义开始结束系统时钟
	start = clock();		//开始解释
	compare_times[0] = 0;	//比较次数
	move_times[0] = 0;		//移动次数
	runtime[0] = 0;			//运行时间
	int i, j;					//i为未排序 j为已排序列表
	for (i = 2; i <= n; i++)	//共n-1趟排序
	{
		r[0] = r[i];			//r[0]为监视哨，也可以作为循环结束标志
		j = i - 1;
		compare_times[0]++;		//增加比较次数
		while (r[j].key > r[0].key)		//比较r[0]，即r[i]和r[j]
		{
			r[j + 1] = r[j];			
			j--;
			move_times[0]++;			//增加移动次数
		}
		r[j + 1] = r[0];				//将r[i]内容插入到r[j]后一位置
	}
	stop = clock();				//记录时间结束
	//显示结果
	/*
